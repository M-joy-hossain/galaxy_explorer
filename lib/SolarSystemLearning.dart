...
class SolarSystemLearning extends StatefulWidget {
  const SolarSystemLearning({super.key});

  @override
  _SolarSystemLearningState createState() => _SolarSystemLearningState();
}

class _SolarSystemLearningState extends State<SolarSystemLearning> with TickerProviderStateMixin {
  ...
  void _setupVideo() {
  _videoController = VideoPlayerController.asset('assets/videos/solar_system_video.mp4')
    ..initialize().then((_) {
      if (mounted) {
        setState(() {
          _isVideoInitialized = true;
        });
        _videoController!.play(); // ইনশিয়ালাইজ হওয়ার পর অটো প্লে হবে
        _videoController!.setLooping(true); // চাইলে লুপ সেট করতে পারেন
      }
